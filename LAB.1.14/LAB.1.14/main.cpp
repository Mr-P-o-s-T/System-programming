...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	si[0].cb = sizeof(si[0]);
	si[1].cb = sizeof(si[1]);
	ZeroMemory(&pi, sizeof(pi) * 2);

	WCHAR a[256] = L"getF 0.0";

	// Start the child process. 
	if (!CreateProcess(L"..\\Debug\\function.exe", a, NULL, NULL, FALSE, 0, NULL, NULL, &si[0], &pi[0])) {
		DWORD error = GetLastError();
		return NULL;
	}

	if (!CreateProcess(L"..\\Debug\\function.exe", a, NULL, NULL, FALSE, 0, NULL, NULL, &si[1], &pi[1])) {
		DWORD error = GetLastError();
		return NULL;
	}
